...
def init_model(conf: Dict):
    ...
    Model = dynamic_load(extractors, conf["model"]["name"])
    model = Model(conf["model"]).eval().to(device)
    logger.info(f"Model {conf['model']['name']} loaded on {device}.")
    return model, device
...
def extract_single_image(
    ...
    """
    对单张图像提取全局描述子，并写入 feature_path 文件。
    """
    name = image_path.stem  # 使用文件名作为组名
    if feature_path.exists() and not overwrite:
        with h5py.File(str(feature_path), "r") as fd:
            if name in fd:
                logger.info(f"Features of {name} already exist, skip.")
                return

    # 读取并预处理图像
    image = read_image(image_path, conf["preprocessing"].get("grayscale", False))
    image = image.astype(np.float32)
    size = image.shape[:2][::-1]

    if conf["preprocessing"].get("resize_max") and (
        conf["preprocessing"].get("resize_force", False) or max(size) > conf["preprocessing"]["resize_max"]
    ):
        scale = conf["preprocessing"]["resize_max"] / max(size)
        size_new = tuple(int(round(x * scale)) for x in size)
        image = resize_image(image, size_new, conf["preprocessing"].get("interpolation", "cv2_area"))
    ...
    # 转换为张量
    tensor_img = torch.from_numpy(image).unsqueeze(0).to(device, non_blocking=True)

    # 模型推理
    pred = model({"image": tensor_img})
    pred = {k: v[0].cpu().numpy() for k, v in pred.items()}

    # 记录原始尺寸
    pred["image_size"] = np.array(size, dtype=np.float32)

    # 如果有 keypoints，进行后处理
    if "keypoints" in pred:
        size_np = np.array(image.shape[-2:][::-1], dtype=np.float32)
        scales = (pred["image_size"] / size_np).astype(np.float32)
        pred["keypoints"] = (pred["keypoints"] + 0.5) * scales[None] - 0.5
        if "scales" in pred:
            pred["scales"] *= scales.mean()
        uncertainty = getattr(model, "detection_noise", 1) * scales.mean()
        pred["keypoints"].attrs["uncertainty"] = uncertainty

    # 半精度转换
    if as_half:
        for k in pred:
            if pred[k].dtype == np.float32:
                pred[k] = pred[k].astype(np.float16)

    # 写入 H5 文件
    feature_path.parent.mkdir(exist_ok=True, parents=True)
    with h5py.File(str(feature_path), "a", libver="latest") as fd:
        if overwrite and name in fd:
            del fd[name]
        grp = fd.create_group(name)
        for k, v in pred.items():
            grp.create_dataset(k, data=v)
...
def main_my(
    ...
    t1 = time.time()
    model, device = init_model(conf, as_half)
    t2 = time.time()
    print(f"Model loading costs {t2 - t1:.3f} s.")
    print("device: ", device)
