...
def load(
    ...
    sfm_dir.mkdir(parents=True, exist_ok=True)
    database = sfm_dir / "database.db"  # SFM目录中的数据库文件路径
    ...
    # 检查数据库是否已经存在
    if database.exists():
        logger.info("Database already exists, skipping creation and import steps.")
        image_ids = {image.name: i for i, image in reference.images.items()}
    else:
    ...
    # 检查是否已经存在重建结果
    model_files = ['images.bin', 'cameras.bin', 'points3D.bin']
    if all((sfm_dir / file).exists() for file in model_files):
        logger.info("Reconstruction already exists, loading from files.")
