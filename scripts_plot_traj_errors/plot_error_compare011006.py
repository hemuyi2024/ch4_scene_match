...
def get_elevpnp_max_values(file_path):
    """
    获取 ElevPnP 的最大误差值（用于设置断层下界）
    """
    _, ex, ey, dist = parse_error_file(file_path)
    return max(ex), max(ey), max(dist)


def plot_multi_error_files_with_break(error_files, save_path="error_compare.png", labels=None):
    assert len(error_files) == len(labels)

    # 读取 ElevPnP 最大误差，用于设置断层位置
    elev_file = None
    for i, label in enumerate(labels):
        if 'elev' in label.lower() or 'pnp' in label.lower():
            elev_file = error_files[i]
            break

    if not elev_file:
        raise ValueError("没有找到 ElevPnP 文件用于自动设定断层")

    elev_ex_max, elev_ey_max, elev_dist_max = get_elevpnp_max_values(elev_file)
    y_split_dict = {
        0: elev_ex_max,
        1: elev_ey_max,
        2: elev_dist_max
    ...
    for i in range(3):
        bax = brokenaxes(
            ylims=((0, y_split_dict[i]), (y_split_dict[i] + 5, y_split_dict[i] + 5 + y_split_dict[i] * 3)),
            hspace=0.05,
            subplot_spec=fig.add_subplot(3, 1, i + 1)
        )
        axs.append(bax)
    ...
    for idx, (file, label, color) in enumerate(zip(error_files, labels, custom_colors)):
        ...
        axs[0].plot(traj, ex, label=label, color=color, marker='o', markersize=1.5, linewidth=2)
        axs[1].plot(traj, ey, label=label, color=color, marker='o', markersize=1.5, linewidth=2)
        axs[2].plot(traj, dist, label=label, color=color, marker='o', markersize=1.5, linewidth=2)
    ...
    for i in range(3):
        ...
        if i == 2:
            axs[i].set_xlabel("Trajectory Length (m)", fontsize=15)
        if i == 0:
            axs[i].legend(loc='upper left', fontsize=13, frameon=True, edgecolor='black')
...
if __name__ == "__main__":
    ...
    plot_multi_error_files(error_files, save_path="/home/lty/paper/results/011006/error_compare0603-0613.png", labels=labels)
